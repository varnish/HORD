...
This development host might not have a RDMA-capable NIC, so HORD is tested against **Soft-RoCE** (the in-kernel `rdma_rxe` software RDMA transport, RoCEv2). It presents a real
...
- `/etc/systemd/system/soft-roce.service` — oneshot unit that recreates the
  `rxe0` link on the ethernet after `network-online.target`. Enabled.

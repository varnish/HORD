...
pub struct Identifier {
    // Behind a Mutex so `migrate` can swap in the new channel: an [`Identifier`]
    // is shared as `Arc`, so it must be mutable through `&self`. Otherwise only
    // read for its `Drop` (keeping the channel alive).
    _event_channel: Mutex<Arc<EventChannel>>,
...
/// Error returned by [`Identifier::migrate`] for moving an identifier to a
/// different event channel.
#[derive(Debug, thiserror::Error)]
#[error("failed to migrate identifier to a new event channel")]
#[non_exhaustive]
...
fn new_cm_id_for_raw(event_channel: Arc<EventChannel>, raw: *mut rdma_cm_id) -> Arc<Identifier> {
    let cm = unsafe {
        Arc::new(Identifier {
            _event_channel: Mutex::new(event_channel),
...
impl Identifier {
    ...
    /// Migrate this identifier to a different [`EventChannel`] (`rdma_migrate_id`).
    /// Afterwards the identifier's events are delivered to `channel` instead of
    /// the one it was created on.
    ///
    /// This lets a server give each accepted connection its own event channel,
    /// decoupled from the listener's — needed when the accept loop and the
    /// per-connection workers run on different threads, so a worker waiting for
    /// `Established` doesn't compete with the acceptor's `get_cm_event`.
    ///
    /// # Note
    ///
    /// Any event already retrieved from the current channel must be acked (e.g.
    /// the `ConnectRequest` that produced this identifier) before migrating.
    pub fn migrate(&self, channel: &Arc<EventChannel>) -> Result<(), MigrateError> {
        let ret = unsafe { rdma_migrate_id(self.cm_id.as_ptr(), channel.channel.as_ptr()) };
